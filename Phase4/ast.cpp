...
bool checkStatement(StateStack* stateStack, ASTNodeStatement* statement, SymbolTable* symTable)
{
	// PHASE3: Assign the current scope to the statement.
	assert(statement->m_Scope == 0);
	statement->m_Scope = symTable->m_CurScope;
...
bool checkFunctionDefinition(StateStack* stateStack, ASTNodeFuncDef* funcDef, SymbolTable* symTable)
{
	...
	funcDef->m_Symbol = funcSymbol;

	stPushScope(symTable, Scope_Function, funcDef->m_LineNumber);
	State* state = pushState(stateStack);
	{
		funcDef->m_Scope = symTable->m_CurScope; // PHASE3
		state->m_IsInsideLoopBody = false; // PHASE3
...
bool checkLValue(StateStack* stateStack, ASTNodeLValue* lvalue, SymbolTable* symTable)
{
	...
	if (type == LValue_Identifier) {
		...
		assert(node->m_Array->m_Type == AST_LValue || node->m_Array->m_Type == AST_Call); // PHASE3
		...
		if (node->m_Array->m_Type == AST_LValue) {
			...
			ASTNodeCall* call = (ASTNodeCall*)node->m_Array;
			if (!checkCall(stateStack, call, symTable)) {
				noErrors = false;
			}
		}
...
bool progBuildSymbolTable(Program* prog)
{
	...
	while (llNode) {
		...
		ASTNodeStatement* statementNode = (ASTNodeStatement*)astNode;
		bool statementOK = checkStatement(&stateStack, statementNode, prog->m_SymbolTable);
		success = !(success && !statementOK);
...
void printScopeSymbols_r(Scope* scope)
{
	...
	while (symbolNode) {
		...
		fprintf(stdout, "%-*s%s %d %s (%d)\n", scope->m_Depth + 1, "", stSymbolTypeToString(symbol->m_Type), symbol->m_LineNumber, symbol->m_Identifier, symbol->m_Offset);
...
void astDestroyLValue(ASTNodeLValue* lvalue)
{
	...
	if (type == LValue_Identifier) {
		...
		if (node->m_Array->m_Type == AST_LValue) {
			...
			astDestroyCall((ASTNodeCall*)node->m_Array);
		}
	} else {
		// Unknown lvalue type.
		assert(false);
